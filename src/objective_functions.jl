...
"Create cost function for a specific owner." 
function create_grid_market_cost_function(model::Ml, T::Int64,
    ...
    grid_cost_owner    = sum(sum(model[:λt][t, thermal_owner[i].number] * model[:p_grid][t, thermal_owner[i].number] for i in 1:Igc) + 
                             sum(model[:λh][t, hydro_owner[j].number] * model[:g_grid][t, hydro_owner[j].number] for j in 1:Jgc) for t in 1:T) 

    market_cost_owner  = sum(sum(model[:λt][t, thermal_owner[i].number] * model[:p_market][t, thermal_owner[i].number] for i in 1:Igc) + 
                             sum(model[:λh][t, hydro_owner[j].number] * model[:g_market][t, hydro_owner[j].number] for j in 1:Jgc) for t in 1:T) 

    grid_cost_system   = sum(sum(PBidt_EQ[t, thermal_system[i].number] * model[:p_grid][t, thermal_system[i].number] for i in 1:Isys) + 
                             sum(PBidh_EQ[t, hydro_system[j].number] * model[:g_grid][t, hydro_system[j].number] for j in 1:Jsys) for t in 1:T) 

    market_cost_system = sum(sum(PBidt_EQ[t, thermal_system[i].number] * model[:p_market][t, thermal_system[i].number] for i in 1:Isys) + 
                             sum(PBidh_EQ[t, hydro_system[j].number] * model[:g_market][t, hydro_system[j].number] for j in 1:Jsys) for t in 1:T)   
